...
// Export to Excel using server-side styled Excel service
...
const handleExcelExport = async () => {
  ...
  try {
    const filters = {
      ...
      'Temp Range': `${minTempFilter.value}°C to ${maxTempFilter.value}°C`,
      'Wind Range': `${minWindFilter.value} to ${maxWindFilter.value} km/h`
    }

    const blob = await api.post('/open-meteo/export', {
      data: filteredCities.value,
      filters
    }, { responseType: 'blob' })
    ...
    const filename = `Weather_Report_${dateStr}_${timeStr}.xlsx`
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-1.5 border-r border-gray-200 pr-3 mr-1">
          <button 
            ...
            class="px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 transition rounded-xl flex items-center gap-1 border-0 cursor-pointer"
          >
            <span v-if="exportingExcel" class="flex items-center gap-1">
