...
<template>
  <div class="fixed inset-0 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md" style="z-index: 100000;">
    <div
      class="bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200"
    >
      <!-- Header -->
      <div class="border-b border-slate-800 p-6 flex justify-between items-center bg-slate-950/40 shrink-0">
        <div>
          ...
          <h2 class="text-2xl font-black text-white mt-0.5 tracking-tight italic">Monthly Planner & Agenda</h2>
        </div>
        <button
          type="button"
      ...
      <div class="flex-1 flex overflow-hidden min-h-0 bg-slate-900">
        ...
        <div class="flex-1 p-6 flex flex-col gap-6 overflow-y-auto">
          <!-- Calendar Nav -->
          <div class="flex justify-between items-center bg-slate-950/50 p-2 rounded-2xl border border-slate-800/80">
            <button
              type="button"
              class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-800 hover:bg-slate-700 text-white transition font-black cursor-pointer"
              @click="navMonth(-1)"
            >
              ←
            </button>
          ...
          <div class="grid grid-cols-7 gap-2">
            <div
              ...
              :key="d"
              class="text-center text-[10px] font-black text-slate-500 uppercase tracking-widest"
            >
              {{ d }}
            </div>
          </div>
            ...
            <button
              ...
              :class="[
                ...
                cell.dateKey === selectedDateKey
                  ? 'bg-indigo-600 hover:bg-indigo-500 font-black'
                  : cell.isToday
                    ? 'bg-slate-800 border-2 border-indigo-500/40 text-indigo-400 font-black'
                    : 'bg-slate-950/30 border border-slate-800/40'
              ]"
        ...
        <!-- Sidebar Agenda Pane -->
        <div class="w-80 border-l border-slate-800 bg-slate-950/30 flex flex-col shrink-0 min-h-0">
          <div class="p-6 border-b border-slate-800/80 bg-slate-950/40">
            <p class="text-[9px] font-black text-slate-500 uppercase tracking-widest">Selected Date</p>
            <h3 class="text-sm font-black text-white mt-1 font-mono">{{ selectedDateLabel }}</h3>
          </div>

          <!-- New Event Input -->
          <div class="p-4 border-b border-slate-800/60 bg-slate-950/10">
            <form @submit.prevent="addEvent" class="flex gap-2">
              <input
                v-model="newEventText"
                type="text"
                placeholder="Add agenda note..."
                class="flex-1 bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white text-xs font-semibold placeholder:text-slate-600 focus:outline-none focus:border-indigo-500"
              />
              <button
                type="submit"
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-2">
            <div
              ...
              :key="idx"
              class="group/item flex items-start justify-between gap-3 bg-slate-900 border border-slate-800/80 p-3 rounded-xl hover:border-slate-700 transition"
            >
              <p class="text-xs text-slate-300 font-semibold leading-normal break-words flex-1 pr-1">
                {{ item }}
              </p>
              <button
                type="button"
                class="text-[10px] text-slate-600 hover:text-rose-500 transition opacity-0 group-hover/item:opacity-100 cursor-pointer"
            ...
            <div
              ...
              class="text-center py-12 text-slate-600 text-xs font-black uppercase tracking-widest"
            >
              No agenda for today
            </div>
          </div>
